...
bool Swapchain::Recreate(uint32_t width, uint32_t height)
{
    ...
    if (auto res = vkCreateSwapchainKHR(vkDevice, &info, nullptr, &swapchain); res != VK_SUCCESS)
...
VkResult Swapchain::AcquireNextImage()
{
    ...
    auto result = vkAcquireNextImageKHR(
      vkDevice, m_swapchain, UINT64_MAX, acquireSemaphore, VK_NULL_HANDLE, &m_currentIndex);
